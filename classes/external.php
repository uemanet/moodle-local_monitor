...
class local_monitor_external extends external_api
{
  ...
  */
  public static function get_online_time_parameters()
  {
    ...
    return new external_function_parameters(array(
      ...
      'grp_id' => new external_value(PARAM_INT, get_string('getonlinetime_param_group_id', 'local_monitor'), 'ID do Grupo', VALUE_DEFAULT)
    ));
  }
  ...
  * @throws Exception
  */
  public static function get_online_time($timeBetweenClicks, $startdate, $enddate, $tutorid, $groupid)
  {
    global $DB;
  ...
  try {
    $tutorGrupo = $DB->get_record('int_tutor_group', array('pes_id' => $tutorid, 'grp_id' => $groupid));

    if(!$tutorGrupo){
      throw new moodle_exception('tutornonexistserror', 'local_monitor', null, null, '');
    }

    $tutor = $DB->get_record('user', array('id' => $tutorGrupo->id));
